	...
	for {
		...
		for _, enw := range enWords {
			...
			enText := fmt.Sprintf("%s %s %s %s %s %s %s %s %s %s", enw.FlMeanings,
				...
				enw.FlDerivate,
				enw.EnPhrases)
			if checkFinished(enw.Id, testDB) {
				continue
			}
...
func checkFinished(gdbId int, dataDb *gorm.DB) bool {
	...
	err := dataDb.Table("check_chinese_idioms").Where("en_id=? and status!=0", gdbId).First(&tmp).Error
	return !errors.Is(err, gorm.ErrRecordNotFound)
}

func checkExisted(gdbId int, dataDb *gorm.DB) bool {
	var tmp CheckEnWords
	err := dataDb.Table("check_chinese_idioms").Where("en_id=?", gdbId).First(&tmp).Error
	return err == nil
}

func createCheck(enId int, enText, rejectMsg string, status int, dataDB *gorm.DB) error {
	return dataDB.Table("check_chinese_idioms").Create(&CheckEnWords{
		EnId:      enId,
		EnText:    enText,
		Status:    status,
		RejectMsg: rejectMsg,
...
func updateCheck(gdbId int, rejectMsg string, status int, dataDB *gorm.DB) error {
	return dataDB.Table("check_chinese_idioms").Where("en_id=?", gdbId).Updates(map[string]interface{}{
		"status":     status,
